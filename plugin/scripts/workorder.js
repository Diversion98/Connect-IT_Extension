$(document).ready(function () {
    ...
    //open Brasserie and send message for brasserie.js om taak af te melden
    chrome.storage.local.get({ brasserie: false }, function (settings) {
        if (settings.brasserie) {
            document.getElementById("close_workorder_btn").addEventListener("click", function () {
                ...
                //send task data to local storage
                chrome.storage.local.set({ taskDetails: taskDetails }, function () {
                    console.log("stored the taskDetails");
                });
            });
        }
    });
